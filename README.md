...
When `~/.codex/thread_history_1.sqlite` exists, the worker waits for the exact turn row and requires it to reach a terminal status. Some user-facing sessions use Codex's legacy history and do not get SQLite rows. For those sessions only, the worker waits for the exact turn's persisted `task_complete` or interrupted `turn_aborted` event in `~/.codex/sessions`. It accepts only user-facing legacy session metadata; internal, paginated, missing, malformed, and ambiguous records remain untracked and do not notify. The hook starts the worker but does not establish that a legacy turn finished. The completed hook event is used as the status only when the database itself does not exist. The same per-turn lock and delivery markers prevent duplicate notifications on both paths.

Persistent marker files contain hashed event and destination identifiers plus delivery outcomes. A temporary private worker request contains the Codex thread ID, turn ID, and hook status; it is deleted after successful delivery, presence suppression, or classification as an untracked hook event. On installations with SQLite history, the persisted SQLite or eligible legacy rollout status controls delivery.
